...
from cons import USER_AGENTS, DISTRO_TO_PACKAGE_MANAGER, PACKAGE_MANAGERS,\
                 ...
                 CURRENT_DIR_PATH, HTTP_PROXIES, HTTPS_PROXIES, BRIDGE_FILES, ASSETS_DIR_PATH
...
def return_error(error_message: str, error_code: int = 400):
    """
    Creates an error response for the client

    :param error_message: The error message with more information about the error
    :param error_code: The error code
    """
    
    return {"status": error_code, "error": error_message, "content": None}, error_code

def load_request_data(asymmetric_encryption: AsymmetricEncryption) -> Tuple[Optional[dict], Optional[dict]]:
    """
    Decrypts and loads the JSON data from the request.

    :param asymmetric_encryption: An instance of the AsymmetricEncryption class.
    """

    if not request.is_json:
        return None, "The query cannot be answered as no data was provided."

    encrypted_request_data = request.json().get("data")
    ...
    try:
        ...
        return None, "An error occurred while loading the given encrypted data."

    public_key = loaded_request_data.get("public_key")
    if public_key is None:
        return None, "The parameter public_key was not given."
    try:
        serialization.load_der_public_key(b64decode(public_key.encode("utf-8")), backend=default_backend())
    except:
        return None, "The public key is corrupted."

    return loaded_request_data, None

def craft_response(data: str, public_key: str):
    """
    Creates an encrypted response for the client

    :param data: Data to be sent to the client
    :param public_key: The client's public key to encrypt the data
    """

    dumped_data = json.dumps(data, indent = None, separators = (',', ':'))
    encrypted_dumped_data = AsymmetricEncryption(public_key).encrypt(dumped_data)

    return {"status": 200, "error": None, "content": encrypted_dumped_data}
...
class Captcha:
    ...
    def generate(self) -> Tuple[str, str]:
        ...
        image_captcha_code = generate_random_string(secrets.choice([8,9,10,11,12]), with_punctuation=False)
        ...
        image_captcha = ImageCaptcha(width=320, height=120, fonts=[
            os.path.join(ASSETS_DIR_PATH, "Comic_Sans_MS.ttf"),
            os.path.join(ASSETS_DIR_PATH, "Droid_Sans_Mono.ttf"),
            os.path.join(ASSETS_DIR_PATH, "Helvetica.ttf")
        ])
